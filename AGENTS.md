...
- Preserve the payload bytes in `test-data/rules-regressions.jsonl.gz`, `test-data/automove-decisions/v1/`, `test-data/automove-decisions/v4/`, and `test-data/compatibility-edge-cases/v1/`. Put revised payloads in a new version directory instead of rewriting existing data.
