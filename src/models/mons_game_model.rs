...
impl MonsGameModel {
    ...
    fn build_scored_root_move_from_transition(
        ...
        let exact_analysis = if simulated_game.active_color == perspective {
            Some(exact_state_analysis(&simulated_game))
        } else {
            None
        };
        ...
        let same_turn_score_window_value = exact_analysis
            ...
            .unwrap_or(0);
        let score_path_best_steps = exact_analysis
            .map(|analysis| {
                analysis
                    .color_summary(perspective)
                    .score_path_window
            ...
            for &target in spirit_loc.reachable_by_spirit_action_ref() {
                ...
                for &dest in target.nearby_locations_ref() {
                    ...
                    let after_same_turn_score_window_value =
                        if after_game.active_color == perspective {
                            exact_state_analysis(&after_game)
                                .color_summary(perspective)
                                .immediate_window
                                .best_score
                                .max(0)
    ...
    fn collect_targeted_same_turn_score_window_inputs(
        ...
        for actor_loc in actor_locations {
            ...
            for transition in actor_transitions {
                ...
                if exact_state_analysis(&transition.game)
                    .color_summary(perspective)
                    .immediate_window
                    .best_score
                    <= 0
        ...
        if exact_turn_before.spirit_assisted_score
            && !root_transitions.iter().any(|transition| {
                transition.game.active_color == perspective
                    && exact_state_analysis(&transition.game)
                        .color_summary(perspective)
                        .immediate_window
                        .best_score
                        > 0
    ...
    fn focused_root_candidates(
        ...
        ranked_indices.sort_by(|a, b| {
            b.1.cmp(&a.1).then_with(|| {
                let candidate = &root_moves[a.0];
                let incumbent = &root_moves[b.0];
        ...
        let mut focused_with_scores = selected_mask
            ...
            .collect::<Vec<_>>();
        focused_with_scores.sort_by(|a, b| {
            b.1.cmp(&a.1).then_with(|| {
                let candidate = &root_moves[a.0];
                let incumbent = &root_moves[b.0];
                if Self::is_better_tactical_root_candidate(candidate, incumbent) {
    ...
    fn pick_root_move_with_exploration(
        ...
        for index in candidate_indices {
            ...
            let equal_spirit_setup_opponent_mana_progress_steps =
                !evaluation.spirit_own_mana_setup_now
                    || !best_spirit_own_mana_setup
                    || !evaluation.opponent_mana_progress
                    || !best_opponent_mana_progress
