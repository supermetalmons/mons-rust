...
import { BOARD_SIZE, type Location } from "../engine/geometry.js";
...
function evaluateStateUtilityFromInputs(
  ...
  if (execution.session.checkpoint()) return EMPTY_TURN_UTILITY;
  const context = createTurnUtilityEvalContext(
    execution,
    start,
    perspective,
    config,
...
export function turnOracleContextWithSearchHash(
  ...
  return turnOracleContextAfterCheckpoint(
    execution,
    game,
    perspective,
    stateHash,
...
function turnOracleContextAfterCheckpoint(
  ...
  const analysis = exactStrategicAnalysisWithSearchHash(
    execution,
    game,
    stateHash,
  );
  ...
  if (execution.session.cacheWriteAllowed) {
    turnEngineCaches(execution).oracle.set(
      stateHash,
      built,
      colorId(perspective),
    );
...
export function opponentCanWinImmediatelyWithSearchHash(
  ...
  if (game.winnerColor() !== undefined || game.activeColor !== opponent)
    return false;
  const needed = saturatingScoreSubtract(
    TARGET_SCORE,
...
function evaluateStateUtilityAfterCheckpoint(
  ...
  const denyGain = saturatingScoreSubtract(
    opponentWindowBefore,
    opponentWindowAfter,
  );
  const drainerSafety = ownDrainerSafetyScore(
    execution,
    game.board,
    perspective,
  );
  const unsafeProgressPenalty =
    drainerSafety < 0
      ? saturatingScoreMultiply(
          Math.min(Math.abs(drainerSafety), MAX_SCORE),
          900,
        )
      : 0;
    ...
    scoreDelta: saturatingScoreSubtract(
      saturatingScoreAdd(
        saturatingScoreAdd(
          saturatingScoreAdd(
            saturatingScoreAdd(
              saturatingScoreMultiply(scoreDelta, 2_400),
              pathBonus,
            ),
            immediateBonus,
          ),
