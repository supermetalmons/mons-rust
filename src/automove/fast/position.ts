...
import { ACTIONS_PER_TURN } from "../../engine/config.js";
...
class MutableFastPosition implements FastPosition {
  ...
  public copyFrom(other: FastPosition): void {
    ...
    this.monLocations.set(source.monLocations);
    this.freeMana.set(source.freeMana);
    this.manaIndices.set(source.manaIndices);
    ...
    this.potions.set(source.potions);
...
function setCell(
  ...
  if (value !== 0) {
    ...
    if (occupancy === OCC_MON) {
      position.monLocations[monId(cellMonKind(value), cellMonColor(value))] =
        index;
    } else if (occupancy === OCC_MANA) {
...
function consumeAction(position: MutableFastPosition): void {
  ...
  position.potions[position.active] =
    i32(position.potions, position.active) - 1;
}
...
function supermanaBackToBase(position: MutableFastPosition): void {
  ...
  if (
    ...
    setCell(
      position,
      SUPERMANA_BASE_INDEX,
      carryingMonCell(existing, MANA_SUPER, 0),
    );
  } else {
...
function applyDemonAction(
  ...
  const demonBase = i32(
    MON_BASE_INDEX,
    monId(cellMonKind(demon), cellMonColor(demon)),
  );
  const representable =
    cellConsumable(target) !== CONS_BOMB ||
    aux === AUX_NONE ||
    aux === demonBase;
  setCell(position, from, 0);
  if (aux === AUX_NONE) {
    ...
    if (targetOccupancy === OCC_MON) {
      ...
      if (destinationOccupancy === OCC_MANA) {
        ...
        setCell(
          position,
          aux,
          carryingMonCell(target, cellMana(destination), 0),
        );
