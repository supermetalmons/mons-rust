...
function threatWalkTable(weight: number, factor: number): Float64Array {
  ...
  for (let steps = 0; steps < THREAT_WALK_TABLE_SIZE; steps += 1) {
    table[steps] = Math.trunc(
      (weight * factor * (MONS_MOVES_PER_TURN + 1 - steps)) / MONS_MOVES_PER_TURN,
    );
  }
  return table;
}
...
export function evaluateWithTables(position: FastPosition, tables: EvalTables): number {
  ...
  for (let color = 0; color < COLOR_COUNT; color += 1) {
    ...
    for (let kind = 0; kind < MON_KIND_COUNT; kind += 1) {
      ...
      if (kind === KIND_DRAINER) {
        ...
        value +=
          sign * i32(tables.drainerCloseToOwnPool, ownPoolDistance(index, color));
        value +=
          sign *
          i32(tables.drainerCloseToSupermana, chebyshev(index, SUPERMANA_BASE_INDEX));
        if (
          carriedMana === 0 &&
        ...
        if (threatSteps === 0) {
          ...
          const walk = carrying ? tables.threatWalkCarrier : tables.threatWalkPlain;
