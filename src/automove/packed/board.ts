...
import { MonKind, type Mon } from "../../api/types.js";
import {
  ALL_GAME_VARIANTS,
  MON_BASE_LOCATIONS,
  SUPERMANA_BASE,
...
export function fastMonCellFromMon(mon: Mon, mana: number, consumable: number): number {
  return makeMonCell(
...
function buildSquares(variant: GameVariant): Uint8Array {
  ...
  for (let index = 0; index < BOARD_CELLS; index += 1) {
    ...
    switch (square.kind) {
      ...
      case "mon-base":
        squares[index] =
          SQ_MON_BASE + monId(MON_KIND_IDS[square.monKind], colorId(square.color));
        break;
    }
  }
  return squares;
...
export const CENTER_ROW_DISTANCE = (() => {
  ...
  for (let index = 0; index < BOARD_CELLS; index += 1) {
    table[index] = Math.abs(BOARD_CENTER_INDEX - Math.trunc(index / BOARD_SIZE));
  }
  return table;
})();
