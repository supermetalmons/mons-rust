...
import { Color, Consumable, type Mana, MonKind, type Square } from "../../api/types.js";
import {
  BOARD_CELLS,
  type Location,
  locationIndex,
...
function squareAllowsEmptyMon(square: Square, monKind: MonKind, color: Color): boolean {
  switch (square.kind) {
    case "regular":
...
export function actorPayloadAfterMoveCompute(
  ...
  switch (payload.kind) {
    ...
    case "mana": {
      if (item === undefined) {
        return squareAllowsManaCarrier(board.squareAt(destination), payload.mana)
          ? payload
          : undefined;
      }
      switch (item.kind) {
        ...
        case "consumable":
          return item.consumable === Consumable.BombOrPotion ? payload : undefined;
      }
    }
    // eslint-disable-next-line no-fallthrough -- the nested exhaustive switch always returns.
    case "bomb": {
