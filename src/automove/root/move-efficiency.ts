import { boardEquals } from "../../engine/board/storage.js";
...
import { Hash64Table, type Hash64 } from "../core/hash64.js";
...
export function distanceToAnyPoolStepsForEfficiency(location: Location): number {
...
function approximateBestDrainerToManaSteps(
  ...
  for (const [drainerLocation, item] of game.board.entries()) {
    ...
    for (const [manaLocation, manaItem] of game.board.entries()) {
      ...
      const candidateSteps = locationDistance(drainerLocation, manaLocation) - 1;
      bestSteps =
        bestSteps === undefined ? candidateSteps : Math.min(bestSteps, candidateSteps);
    }
  }
  return bestSteps;
}
...
function initializeMoveEfficiencySnapshot(
  ...
  return {
    ...
    opponentBestCarrierSteps: opponentSummary?.bestCarrierSteps ?? UNKNOWN_STEPS,
  ...
  for (const [location, item] of game.board.entries()) {
    if (item.kind === "mon-with-mana") {
      ...
      if (item.mon.color === perspective) {
        ...
        snapshot.myBestCarrierSteps = Math.min(snapshot.myBestCarrierSteps, poolSteps);
      } else {
    ...
    if (item.mon.color === perspective) {
      ...
      snapshot.opponentSpiritOnBase = locationEquals(location, opponentSpiritBase);
    }
  }
}
...
export function clearMoveEfficiencyCache(context: AutomoveExecutionContext): void {
...
export function manaHandoffPenalty(
  ...
  for (const event of events) {
    ...
    const myBefore = distanceToColorPoolStepsForEfficiency(event.from, perspective);
    const myAfter = distanceToColorPoolStepsForEfficiency(event.to, perspective);
    const opponentBefore = distanceToColorPoolStepsForEfficiency(event.from, opponent);
    const opponentAfter = distanceToColorPoolStepsForEfficiency(event.to, opponent);
    ...
    if (movedTowardOpponent > movedTowardMe) {
      ...
      penalty = penalty + excess * manaScore(event.mana, opponent) * perStepPenalty;
    }
  }
  return penalty;
}
...
function applyCarrierAndDrainerEfficiencyDelta(
  ...
  delta =
    delta +
    stepProgressDelta(before.myBestCarrierSteps, after.myBestCarrierSteps, 90, 130);
  delta =
    delta -
    stepProgressDelta(
  ...
  delta = delta - (after.opponentCarrierCount - before.opponentCarrierCount) * 48;
  return delta;
}
...
function applyScoreWindowEfficiencyDelta(
  ...
  let delta = initialDelta;
  delta = delta + (after.mySameTurnScoreValue - before.mySameTurnScoreValue) * 55;
  delta =
    delta - (after.opponentSameTurnScoreValue - before.opponentSameTurnScoreValue) * 45;
  delta =
    delta +
    (after.mySameTurnOpponentManaScoreValue - before.mySameTurnOpponentManaScoreValue) *
      90;
  delta =
    delta -
...
function applySafeProgressEfficiencyDelta(
  ...
  if (!before.opponentSafeSupermanaProgress && after.opponentSafeSupermanaProgress) {
    delta = delta - 120;
  }
...
function applyRootEfficiencyPenalties(
  ...
  if (policy.applyRootManaHandoffGuard && !rootCompensatesHandoff) {
    delta =
      delta - manaHandoffPenalty(events, perspective, policy.rootManaHandoffPenalty);
  }
