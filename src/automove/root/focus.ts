...
import { AUTOMOVE_TURN_ENGINE_MODE } from "../config/types.js";
import type { RootFocusCandidate, RootFocusConfig } from "./focus-model.js";
...
type RootFocusOptions<Candidate extends RootFocusCandidate> = {
  ...
  readonly qualifiesDrainerSafetyRecoveryPlan?: (candidate: Candidate) => boolean;
...
function rootProgressStepSoftBonus(steps: number, perStepBonus: number): number {
  const unknownSteps = BOARD_SIZE + 4;
  if (steps >= unknownSteps || perStepBonus <= 0) return 0;
  const clampedSteps = Math.min(MONS_MOVES_PER_TURN, Math.max(0, Math.trunc(steps)));
...
export function rootProgressStepsBetter(
  ...
  return candidateKnown ? !incumbentKnown || candidateSteps < incumbentSteps : false;
}
...
function isBetterTacticalRootCandidate(
  ...
  if (candidate.scoresOpponentManaThisTurn !== incumbent.scoresOpponentManaThisTurn) {
  ...
  if (candidate.safeOpponentManaPickupNow !== incumbent.safeOpponentManaPickupNow) {
  ...
  if (candidate.spiritSameTurnScoreSetupNow !== incumbent.spiritSameTurnScoreSetupNow) {
...
function compareTacticalRootCandidateOrder(
  ...
  if (isBetterTacticalRootCandidate(candidate, incumbent, compareSpiritScorePath)) {
    return -1;
  }
  if (isBetterTacticalRootCandidate(incumbent, candidate, compareSpiritScorePath)) {
    return 1;
  }
  return 0;
}
...
export function prioritizeRootInputs<Candidate extends RootFocusCandidate>(
  ...
  if (priorityInputs !== undefined) {
    for (const inputs of priorityInputs) {
      if (!orderedInputs.some((existing) => inputChainsEqual(existing, inputs))) {
...
function rankedRootIndices(
  ...
  ranked.sort((left, right) => compareRankedRootIndices(rootMoves, left, right));
  return ranked;
}
...
function selectFocusedRootIndices<Candidate extends RootFocusCandidate>(
  ...
  const selected = Array.from<boolean>({ length: rootMoves.length }).fill(false);
  ...
  if (config.search.volatilityFocus) {
    ...
    if (bestVolatility !== undefined) {
      for (const { index, volatility, scoutScore } of volatilityRanked) {
        if (volatility + ROOT_FOCUS_CONSTANTS.volatilityMargin < bestVolatility) {
          break;
        }
        if (scoutScore + ROOT_FOCUS_CONSTANTS.focusScoreMargin < effectiveBestScore) {
          continue;
        }
        selected[index] = true;
      }
...
export function focusedRootCandidates<Candidate extends RootFocusCandidate>(
  ...
  const focusCount = Math.min(ROOT_FOCUS_CONSTANTS.focusCount, rootMoves.length);
  const rankedIndices = rankedRootIndices(rootMoves, scout.scores);
  if (hasNarrowScoutSpread(rankedIndices, focusCount)) {
    const rankedRoots = rankedIndices.map(([index]) => valueAt(rootMoves, index));
  ...
  const focusedWithScores: RankedRootIndex[] = selected.flatMap((isSelected, index) => {
  ...
  return {
    ...
    scoutVisitedNodes: Math.min(scout.visitedNodes, config.budget.maxVisitedNodes),
  };
}
