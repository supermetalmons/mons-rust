...
import {
  ...
  orderingEventBonus,
} from "../root/observations.js";
import { compareRootCandidates } from "../root/ranking.js";
import { isOwnDrainerVulnerable } from "../root/vulnerability.js";
import { rankRootCandidates, terminalSearchScore } from "../root/candidates.js";
import type { EvaluatedRoot, RootCandidate, SearchConfig } from "../root/types.js";
import {
  clearMoveEfficiencyCache,
  moveEfficiencyDeltaFromBeforeSnapshot,
  moveEfficiencySnapshotWithHash,
} from "../root/move-efficiency.js";
...
import { isQuiescenceTacticalTransition } from "../transitions/event-classification.js";
...
export type SearchRootOptions = {
  ...
  readonly qualifiesDrainerSafetyRecoveryPlan?: (candidate: RootCandidate) => boolean;
};
...
function preferabilityCache(execution: AutomoveExecutionContext): Hash64Table<number> {
...
function rankedChildren(
  ...
  for (const transition of enumerateLegalTransitions(
    ...
    const orderingEfficiency = moveEfficiencyDeltaFromBeforeSnapshot(
      ...
      hash,
      {
        ...
        rootManaHandoffPenalty: context.config.evaluation.rootManaHandoffPenalty,
      },
    );
    let heuristic =
      terminalSearchScore(
    ...
    if (preferredChildHash !== undefined && hash64Equals(hash, preferredChildHash)) {
  ...
  children.sort((left, right) => compareRankedChildren(left, right, maximizing));
...
function boundedSearch(
  ...
  if (
    ...
    if (
      (maximizing &&
        saturatingScoreAdd(evaluation, context.config.search.futilityMargin) < alpha) ||
      (!maximizing &&
        saturatingScoreSubtract(evaluation, context.config.search.futilityMargin) >
          beta)
    ) {
      return evaluation;
    }
  ...
  if (
    ...
    if (
      context.transposition.size >= context.config.search.transpositionCapacity &&
  ...
  return {
    ...
    extensionNodeBudget: config.search.selectiveExtensions
      ? Math.max(
          1,
          Math.floor(
            (config.budget.maxVisitedNodes * config.budget.extensionNodeShareBp) /
              10_000,
          ),
        )
      : 0,
...
function candidateWithCurrentStateHash(candidate: RootCandidate): RootCandidate {
...
function betterEvaluation(candidate: EvaluatedRoot, incumbent: EvaluatedRoot): boolean {
  return (
