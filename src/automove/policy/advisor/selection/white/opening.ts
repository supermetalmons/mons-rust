...
function whiteSetupProgressCompetitionOverride(
  ...
  if (approvedUtility === undefined || approvedFollowup === undefined) return undefined;
  ...
  for (const index of selectionIndices) {
    ...
    const utilityOrder = compareUtilityPrimaryAxes(challengerUtility, currentUtility);
    const replace =
      utilityOrder > 0 ||
...
function whiteEarlySetupSiblingProgressOverride(
  ...
  return bestOverrideIndex(
    ...
    (left, right) => {
      ...
      if (leftRoot === undefined || rightRoot === undefined) return left - right;
      const leftStepDelta =
        approved.safeSupermanaProgressSteps - leftRoot.safeSupermanaProgressSteps;
      const rightStepDelta =
        approved.safeSupermanaProgressSteps - rightRoot.safeSupermanaProgressSteps;
