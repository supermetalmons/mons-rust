...
import { isBlackTurnSixPlainSpiritSetupPair } from "./representatives.js";
import { compareRootMoveSearchPriority, rootMoveUtility } from "./support.js";
import type {
...
function injectedCandidatePassesRootGate(
  ...
  const top = roots[0];
  if (top === undefined || (top.winsImmediately && !candidate.winsImmediately)) {
    return false;
  }
  ...
  const projectedNearCompletion =
    ...
    (projectedFinished ||
      ...
      !projected.game.playerCanMoveMon() ||
      (!projected.game.playerCanUseAction() && !projected.game.playerCanMoveMana()));
  ...
  const regressesPlainSpiritCluster =
    ...
    roots.slice(0, 3).some((root) => {
      ...
      return (
        ...
        root.safeSupermanaProgressSteps <= candidate.safeSupermanaProgressSteps &&
        root.safeOpponentManaProgressSteps <= candidate.safeOpponentManaProgressSteps &&
        compareUtilityPrimaryAxes(
          rootMoveUtility(execution, game, root, perspective, config),
          plan.headUtility,
        ) >= 0
  ...
  const utilityOverride =
    roots.slice(0, 3).every((root) => {
      const utility = rootMoveUtility(execution, game, root, perspective, config);
      return (
  ...
  if (progressHead && !hasProgressSurface(candidate) && !completedPlanOverride) {
    return false;
  }
  if (
    !utilityOverride &&
...
export function evaluateInjectedRoot(
  execution: AutomoveExecutionContext,
  game: MonsGame,
  perspective: Color,
  config: AutomoveConfig,
  ...
  const existing = roots.find((root) => inputChainsEqual(root.inputs, candidateInputs));
  const candidate =
    existing ??
  ...
  if (!admitted) {
    const incumbentUtility = rootMoveUtility(execution, game, top, perspective, config);
    ...
    const resolvesSurface =
      ...
      (simulatedCandidate.attacksOpponentDrainer && !top.attacksOpponentDrainer) ||
      ...
      (simulatedCandidate.sameTurnScoreWindowValue > top.sameTurnScoreWindowValue &&
    ...
    const sameOpeningFollowup =
      ...
      roots
        .slice(0, 4)
        .filter((root) =>
          isBlackTurnSixPlainSpiritSetupPair(game, simulatedCandidate, root, config),
        ).length >= 2 &&
      compareUtilityPrimaryAxes(candidateUtility, incumbentUtility) >= 0 &&
