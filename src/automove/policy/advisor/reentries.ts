...
import { productionWhiteTurnFourManaSiblingReentry } from "../reply-risk/reentry.js";
import { sameNonTacticalProgressLane } from "../reply-risk/shortlist.js";
import { spiritFollowupFloorScore } from "../reply-risk/projection.js";
import { whiteSpiritFollowupSetupCompetition } from "../reply-risk/competition.js";
...
function findScoredRepresentative(
  ...
  const anchorUtility = rootUtility(execution, game, anchor, perspective, config);
  return roots
    ...
    .sort((left, right) => compareRankedEvaluatedRootIndices(roots, left, right))[0];
}
...
function plainSpiritClusterProgressReentry(
  ...
  const followupReentry = omitted
    ...
    .sort((left, right) => {
      ...
      return followupOrder || compareRankedEvaluatedRootIndices(roots, left, right);
    })[0];
...
function blackTurnSixSpiritReentry(
  ...
  const bestRank = Math.min(
    ...shortlist.map((index) => roots[index]?.rootRank ?? Number.MAX_SAFE_INTEGER),
  );
...
function blackNoActionSafeProgressReentry(
  ...
  const delta = exactOpportunityContext(execution, game, game.activeColor).delta;
  if (
    delta.sameTurnScoreWindowValue > 1 ||
    delta.opponentWindowDenyGain > 1 ||
    (delta.sameTurnScoreWindowValue === 0 && delta.opponentWindowDenyGain === 0) ||
  ...
  return roots
    ...
    .filter((index) => {
      ...
      if (
        ...
        root.safeOpponentManaProgressSteps === anchor.safeOpponentManaProgressSteps &&
...
function collectAdvisorReentries(
  ...
  add(
    riskyRecoveryReentry(execution, game, roots, candidateIndices, perspective, config),
  );
  ...
  add(blackNoActionSafeProgressReentry(execution, game, roots, shortlist, config));
