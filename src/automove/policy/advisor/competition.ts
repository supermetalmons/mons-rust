...
import { spiritProjectionChallengeOrder } from "../reply-risk/spirit-ordering.js";
import { rootFamily as advisorRootFamily } from "../../root/family.js";
import {
  compareRankedEvaluatedRootIndices,
  spiritScoreChallengeOrder,
} from "../../root/evaluated-ordering.js";
import type { RootSelectionContext } from "../../root/selector-model.js";
...
function spiritSetupCompetes(
  ...
  if (
    ...
    candidate.spiritSetupGain >= saturatingScoreAdd(incumbent.spiritSetupGain, 48) &&
...
function safeProgressCompetition(context: RootSelectionContext): boolean {
  ...
  const spiritEfficiencies = context.candidateIndices
    ...
    .filter(
      (root): root is EvaluatedRoot =>
        root !== undefined && root.spiritDevelopment && !root.manaHandoffToOpponent,
    )
  ...
  return context.candidateIndices.some((index) => {
    ...
    return (
      ...
      spiritIndices.every((spiritIndex) => {
        ...
        return (
          spirit !== undefined && (spiritScoreChallengeOrder(root, spirit) ?? 0) > 0
        );
      })
    );
  });
...
function projectionCompetition(
  ...
  const bestProjection =
    bestSpiritIndex === undefined ? undefined : projections.get(bestSpiritIndex);
  if (
  ...
  return context.candidateIndices.some((index) => {
    ...
    const safeChallenger = canChallengeSpiritPreferenceRoot(root, context.perspective);
    ...
    return (
      ...
      (spiritProjectionChallengeOrder(root, projection, bestSpirit, bestProjection) ??
        0) > 0
    );
  });
}
...
function negativeDenyCompetition(
  ...
  const spiritIndices = projectedSpiritIndices(context, projections).slice(0, 3);
  if (
...
function riskyRecoveryCompetition(
  ...
  const spiritUtilities = spiritIndices
    ...
    .map((root) =>
      rootUtility(execution, context.game, root, context.perspective, context.config),
    );
