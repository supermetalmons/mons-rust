...
import { rootProgressOrSetupBetter } from "./shortlist.js";
...
export function blackPlainSpiritFollowupReplyOrder(
  ...
  if (
    ...
    (plainFollowup < saturatingScoreAdd(setupFollowup, 32) && plain.score < setup.score)
  ) {
    return undefined;
  }
...
export function earlyBlackPlainSpiritSiblingOrder(
  ...
  if (candidateBetter === incumbentBetter) {
    ...
    if (candidateClose && incumbentClose && candidate.score !== incumbent.score) {
...
export function earlyBlackManaProgressReplyOrder(
  ...
  if (candidateWindow !== incumbentWindow) {
    ...
    const windowSnapshot = candidateWindow ? candidateSnapshot : incumbentSnapshot;
    const progress = candidateWindow ? incumbent : candidate;
    const progressSnapshot = candidateWindow ? incumbentSnapshot : candidateSnapshot;
    if (
...
export function safeNonSpiritFollowupOrder(
  ...
  const challengerSnapshot = candidatePlain ? incumbentSnapshot : candidateSnapshot;
...
export function whiteSpiritFollowupSetupReplyOrder(
  ...
  const floorMargin = game.turnNumber === 3 && game.monsMovesCount === 1 ? 192 : 96;
  if (
