...
export function compareRankedReplyRiskEvaluations(
  ...
  return (
    ...
    compareTacticalEvaluatedRoots(left, right) ||
...
export function rankedRootOrder(
  ...
  const tactical = compareTacticalEvaluatedRoots(candidate, incumbent);
...
export function sameNonTacticalProgressLane(
  ...
  const sameProgressSteps =
    candidate.safeSupermanaProgressSteps === anchor.safeSupermanaProgressSteps &&
    candidate.safeOpponentManaProgressSteps === anchor.safeOpponentManaProgressSteps;
...
export function replyRiskGuardShortlistIndices(
  ...
  if (candidateIndices.length === 0 || execution.session.checkpoint()) return [];
  ...
  let shortlist = candidateIndices.filter((index) => {
    const root = evaluations[index];
    return (
      root !== undefined && saturatingScoreAdd(root.score, scoreMargin) >= bestScore
    );
  });
  ...
  if (shortlist.length > shortlistLimit) {
    if (productionEnabled(config)) {
      ...
      const bestShortlistScore = evaluations[shortlist[0] ?? -1]?.score ?? bestScore;
      const extras: number[] = [];
  ...
  const anchor = anchorIndex === undefined ? undefined : evaluations[anchorIndex];
  if (
