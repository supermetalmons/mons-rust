...
import { TURN_PLAN_FAMILY_PRIORITY_ORDER, TurnPlanFamily } from "../../turn/model.js";
import {
...
import type { RootReplyRiskSnapshot, TurnEngineRootProjection } from "./types.js";
import {
...
export function compareSpiritProjectionPlans(
  ...
  if (closeSpiritRoots) {
    ...
    order = compareUtilityPrimaryAxes(candidate.plan.utility, incumbent.plan.utility);
    if (order !== 0) return order;
    const candidateImpact = candidate.plan.headFamily === TurnPlanFamily.SpiritImpact;
    const incumbentImpact = incumbent.plan.headFamily === TurnPlanFamily.SpiritImpact;
...
export function mixedPlainSpiritReplyFloorOrder(
  ...
  const nonSpiritSnapshot = candidateImpact ? incumbentSnapshot : candidateSnapshot;
  const nonSpiritProjection = candidateImpact
    ? incumbentProjection
    : candidateProjection;
  const spiritSnapshot = candidateImpact ? candidateSnapshot : incumbentSnapshot;
  const spiritProjection = candidateImpact ? candidateProjection : incumbentProjection;
  ...
  if (
    spiritPriority > closeSpiritGoalFamilyPriority(TurnPlanFamily.SpiritImpact) ||
...
export function mixedPlainSpiritProjectionOrder(
  ...
  const nonSpiritRootCompetes =
    ...
    spiritGoalPriority <= closeSpiritGoalFamilyPriority(TurnPlanFamily.SpiritImpact) &&
  ...
  const nonSpiritCompetes =
    (nonSpiritProjection.plan.headFamily === TurnPlanFamily.SafeSupermanaProgress ||
      nonSpiritProjection.plan.headFamily === TurnPlanFamily.SafeOpponentManaProgress ||
      nonSpiritProjection.plan.headFamily === TurnPlanFamily.DrainerSafetyRecovery) &&
    (followupOrder === 0 ||
      utilitySupportsFamilyFallback(
        nonSpiritProjection.plan.utility,
        spiritProjection.plan.utility,
...
export function spiritProjectionChallengeOrder(
  ...
  const spiritProjection = candidatePlain ? candidateProjection : incumbentProjection;
