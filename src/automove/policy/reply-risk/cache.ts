...
function replyRiskCaches(execution: AutomoveExecutionContext): ReplyRiskSessionCaches {
...
function cacheGet<V>(cache: Hash64Table<V>, key: ReplyRiskCacheKey): V | undefined {
  return cache.get(key.hash, key.tag, key.secondary, key.qualifier);
}

function cacheSet<V>(cache: Hash64Table<V>, key: ReplyRiskCacheKey, value: V): void {
...
export function selectedOverrideConfigKey(
  ...
  if (
    fields.some(
      ([value, maximum]) => !Number.isInteger(value) || value < 0 || value > maximum,
    )
  ) {
    return undefined;
  }
