...
export function lateSafeManaRootOrder(
  ...
  } else if (candidate.ownDrainerVulnerable && !incumbent.ownDrainerVulnerable) {
    safe = incumbent;
...
function normalRootSafetySnapshot(
  ...
  for (const reply of replies) {
    ...
    if (replyWinner === perspective) {
      ...
      replyScore = evaluateReplyRiskGame(execution, afterReply, perspective, config);
    }
...
function betterNormalRootSafetyCandidate(
  ...
  if (candidate.allowsImmediateOpponentWin !== incumbent.allowsImmediateOpponentWin) {
    return !candidate.allowsImmediateOpponentWin;
  }
  if (candidate.opponentReachesMatchPoint !== incumbent.opponentReachesMatchPoint) {
...
function normalRootSafetyDeepFloorScore(
  ...
  if (stateAfterMove.activeColor === perspective) {
    return evaluateReplyRiskGame(execution, stateAfterMove, perspective, config);
  }
  ...
  const nodeBranchLimit = clamp(Math.max(0, config.search.nodeBranchLimit - 4), 4, 10);
...
export function normalSafetyReplyOrder(
  ...
  const axesDiffer =
    ...
    candidateNormal.opponentMaxScoreGain !== incumbentNormal.opponentMaxScoreGain ||
  ...
  const opponent =
    TARGET_SCORE -
    scoreForColor(game, perspective === Color.White ? Color.Black : Color.White);
  if (config.replyRisk.deepSafetyFloor && (mine <= 3 || opponent <= 3)) {
    const floor = (state: MonsGame): number =>
      normalRootSafetyDeepFloorScore(execution, state, perspective, config, replyLimit);
