...
import { compareTurnUtilities } from "../../turn/ordering.js";
import {
...
import { TERMINAL_SEARCH_SCORE as SMART_TERMINAL_SCORE } from "../../core/score-math.js";
import { projectionTurnEngineConfig } from "./config.js";
import {
...
export function canTurnEngineProjectReplyRiskRoot(
  ...
  return root.game.activeColor === perspective && root.game.winnerColor() === undefined;
}
...
function isInformativeReplyRiskProjectionFamily(family: TurnPlanFamily): boolean {
  return (
...
export function rootReplyRiskSnapshotWithProjection(
  ...
  const state =
    ...
    shouldUseReplyRiskProjectionForRoot(root, projection, perspective, config)
      ...
      : root.game;
  return rootReplyRiskSnapshot(execution, state, perspective, config, replyLimit);
}
...
export function turnEngineReplyRiskProjections(
  ...
  if (hasSpiritDevelopmentWindow) {
    ...
    projectionLimit = Math.min(shortlist.length, config.planner.lowBudgetGuard ? 3 : 6);
  }
  ...
  for (const index of shortlist.slice(0, projectionLimit)) {
    ...
    const root = evaluations[index];
    if (root === undefined || !canTurnEngineProjectReplyRiskRoot(root, perspective)) {
      continue;
    }
    const vulnerableRecoveryProjection =
      ...
      !root.hasRoundtrip;
    const engineConfig = vulnerableRecoveryProjection ? fullConfig : rerankConfig;
...
export function buildSpiritRootProjections(
  ...
  const spiritShortlist = candidateIndices
    ...
    .sort((left, right) => compareRankedReplyRiskEvaluations(evaluations, left, right))
    .slice(0, spiritLimit);
