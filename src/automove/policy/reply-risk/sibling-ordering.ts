import { inputChainsShareFirstInput } from "../../../engine/model/domain.js";
...
import { isSafePlainSpiritPair, sameNonTacticalProgressLane } from "./shortlist.js";
...
export function riskyRecoveryProgressSiblingOrder(
  ...
  const riskyProjection = candidateUnsafe ? candidateProjection : incumbentProjection;
  const safe = candidateUnsafe ? incumbent : candidate;
  const safeSnapshot = candidateUnsafe ? incumbentSnapshot : candidateSnapshot;
  const safeProjection = candidateUnsafe ? incumbentProjection : candidateProjection;
  if (
...
export function safePickupOrder(
  ...
  const pickupSnapshot = candidatePickup ? candidateSnapshot : incumbentSnapshot;
