...
import { commitPlanAndSeedFollowup } from "./plan-commit.js";
import {
...
function smartSearchBestInputsInternal(
  ...
  if (config.planner.enabled) {
    ...
    const skipHead = shouldSkipProductionHeadPlanForRootContext(game, roots, config);
    const headPlan = skipHead
      ? undefined
      : productionUseFreshLiveHeadPlan(game, config)
        ? turnEngineCandidatePlanLive(execution, game, perspective, engineConfig)
  ...
  const priority =
    advisor === undefined ? [] : productionRootAdvisorPriorityInputs(advisor);
  return searchAndSelectRoot(execution, game, perspective, config, roots, priority)
    .inputs;
}
