...
import { rootProgressStepsBetter } from "../../root/focus.js";
import {
...
import { TurnEngineMode, TurnPlanFamily, type TurnPlan } from "../../turn/model.js";
import {
...
function bestTacticalRootIndex(
  ...
  roots.forEach((root, index) => {
    if (
      predicate(root) &&
      (best === undefined ||
        compareTacticalRootCandidatesIgnoringScorePath(root, valueAt(roots, best)) < 0)
    ) {
      best = index;
    }
  });
...
export function forcedTacticalPrepassChoice(
  ...
  const choose = (predicate: (root: RootCandidate) => boolean): Input[] | undefined => {
  ...
  if (!hasException && ownDrainerVulnerableNextTurn(execution, game, perspective)) {
    choice = choose((root) => !root.ownDrainerVulnerable);
    if (choice !== undefined) return choice;
  }
  const opponentScore = perspective === Color.White ? game.blackScore : game.whiteScore;
...
export function acceptTurnEngineCachedStep(
  ...
  const index = roots.findIndex((root) => inputChainsEqual(root.inputs, cachedInputs));
...
export function forcedLowBudgetTurnEnginePrepassChoice(
  ...
  const index = roots.findIndex((root) => inputChainsEqual(root.inputs, inputs));
...
export function shouldInvokeTurnHeadRerank(roots: readonly RootCandidate[]): boolean {
  ...
  return (
    roots[0] !== undefined && (tactical(roots[0]) || roots.slice(0, 3).some(tactical))
  );
}
...
export function classifyTurnEngineRerankOverride(
  ...
  const decisive =
    ...
    candidate.safeOpponentManaPickupNow;
  const soft =
    candidate.sameTurnScoreWindowValue > 0 || candidate.spiritSameTurnScoreSetupNow;
  const gap = saturatingScoreSubtract(top.heuristic, candidate.heuristic);
  const materialAdvantage =
    (candidate.attacksOpponentDrainer && !top.attacksOpponentDrainer) ||
    ((candidate.scoresSupermanaThisTurn || candidate.scoresOpponentManaThisTurn) &&
  ...
  const progress =
    candidate.supermanaProgress || candidate.opponentManaProgress || progressBetter;
  const progressOnly =
    progress &&
    !decisive &&
    !soft &&
  ...
  const safetyProgressFallback = safer && progressBetter && index <= 5 && gap <= 220;
  if (decisive || soft || candidate.classes.drainerSafetyRecover || progress) {
    if (candidate.ownDrainerVulnerable && !candidate.classes.drainerSafetyRecover) {
      return false;
    }
    const signal =
      materialAdvantage ||
