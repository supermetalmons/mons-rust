...
import { isProductionModeBlackPlainSpiritFollowupSetupPair } from "../reply-risk/followup-ordering.js";
...
import { turnEngineModeFromAutomoveConfig as modeFromConfig } from "../../turn/config.js";
import { turnEngineModeUsesMacroPlans } from "./config.js";
...
export function acceptTurnEngineHeadAfterSearch(
  ...
  const exactContext = exactOpportunityContext(execution, game, game.activeColor);
  ...
  let selectedUtilityCache: ReturnType<typeof turnEngineSelectedUtility> | null = null;
  const selectedUtilityValue = (): ReturnType<typeof turnEngineSelectedUtility> => {
  ...
  let candidateUtilityCache: ReturnType<typeof turnEngineSelectedUtility> | null = null;
  const candidateUtilityValue = (): ReturnType<typeof turnEngineSelectedUtility> => {
  ...
  const whiteSpiritSetupGain =
    ...
    candidate.spiritSetupGain >= saturatingScoreAdd(selected.spiritSetupGain, 32) &&
