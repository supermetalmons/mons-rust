...
import { TurnEngineMode, type TurnEngineConfig } from "./model.js";
...
export function productionSecondaryAnalysisLive(config: AutomoveConfig): boolean {
  return (
...
function applyTurnEngineRerankLimits(config: TurnEngineConfig): TurnEngineConfig {
...
export function turnEngineRerankConfigFromAutomoveConfig(
  ...
  return applyTurnEngineRerankLimits(turnEngineConfigFromAutomoveConfig(config));
}
