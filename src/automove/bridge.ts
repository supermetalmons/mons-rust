import { Consumable, Modifier, type Mana } from "../api/types.js";
import {
  MON_KIND_IDS,
  colorId,
  itemMana,
...
import { BOARD_CELLS, fromLocationIndex } from "../engine/board/geometry.js";
import {
  ...
  for (let index = 0; index < BOARD_CELLS; index += 1) {
    ...
    switch (item.kind) {
      ...
      case "mon-with-consumable":
        cells[index] = fastMonCellFromMon(item.mon, 0, consumableCode(item.consumable));
        break;
      case "consumable":
...
function modifierInput(modifier: number): Input {
  return {
    kind: "modifier",
    modifier: modifier === MOD_BOMB ? Modifier.SelectBomb : Modifier.SelectPotion,
  };
}

export function moveToInputs(move: number): Input[] {
  const type = moveType(move);
  const inputs: Input[] = [locationInput(moveFrom(move)), locationInput(moveTo(move))];
  if (type === MOVE_SPIRIT) {
