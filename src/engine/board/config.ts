...
function buildSquares(variant: GameVariant): readonly Square[] {
  const squares = Array.from<Square>({ length: BOARD_CELLS }).fill(REGULAR_SQUARE);
...
function buildInitialItems(variant: GameVariant): readonly (Item | undefined)[] {
  return Object.freeze(squaresForVariant(variant).map(initialItemForSquare));
}

const INITIAL_ITEMS_BY_VARIANT = new Map<GameVariant, readonly (Item | undefined)[]>(
...
export function initialItemsForVariant(variant: GameVariant): (Item | undefined)[] {
