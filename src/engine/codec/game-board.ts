...
import { BOARD_CELLS, BOARD_SIZE } from "../board/geometry.js";
import { Color, type Item } from "../model/domain.js";
...
export function gameFen(game: GameFenState): string {
  ...
  return variant === DEFAULT_GAME_VARIANT ? fen : `${fen} ${gameVariantId(variant)}`;
}
...
export function parseGameFen(fen: string): GameFenState | undefined {
  ...
  const activeColor =
    colorCode === 119 ? Color.White : colorCode === 98 ? Color.Black : undefined;
