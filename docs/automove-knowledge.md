...
- `black_recovery_branch`: not a root-availability problem. The current live model prefers the approved plain-spirit root or a no-guard ProV1 spirit replay over shipping under frontier metrics. Static exact evaluation flips the ordering, so scoped exact-static cost/reliability is the only live follow-up.
