...
- A scorer-local board summary can be directionally useful, but if it builds heap `Vec`s per evaluation it can become the next visible search-side wall itself. In the latest ProV2 wave, `v50` helped move the direct blocker off the scorer and back into the oracle exact surface, but `v51` showed `ScoringBoardSummary::from_board` and its allocation/drop churn immediately once the oracle cut got stronger.
- One clone per spirit-position plus preview mutate/undo is a safe replacement for one clone per spirit-preview inside `exact_tactical_spirit_summary`. In the latest ProV2 wave, `v51`'s `apply_spirit_move_preview_in_place` cut the bounded hotspot again and was strong enough to move the live direct wall back to the search side, even though it still was not enough to finish `pro-reliability`.
- When an exact-path cut returns the direct wall to search-side scoring, first check whether the new helper you just introduced is now the visible cost before iterating deeper on that returned surface. In the latest ProV2 wave, `v51` returned the wall to search-side scoring, but the sample showed the new board-summary helper itself before another search-side split was justified.
