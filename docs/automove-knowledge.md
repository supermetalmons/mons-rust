...
- `white_search_order_rank_surface_probe` closes the next obvious negative-deny follow-up. On the unresolved Normal sibling, shipping `l9,4;l8,5` is both `selected_rank=0` and `root_rank=0`. On the retained vulnerable guard, shipping `l8,4;l8,5` still shows `selected_rank=4` under the shipping runtime, but its underlying `root_rank` is already `0`; the raw shipping-own-caps replay also gives that same guard move `root_rank=0`.
- So neither current selected-rank nor current root-rank surface is a safe separator for the remaining negative-deny white seam. Wrapper cuts that gate on “shipping move is rank 0” or “capped search-only move is rank 0” still reopen the retained vulnerable guard before any duel spend.
- So the white wrapper frontier is now sharper, not closed. Search-only `ProV1` semantics remain the real local explanation, but the kept runtime use is only the nonnegative-deny Fast sibling. The remaining negative-deny seams still need a deeper shortlist/root-scoring theory rather than another broad branch into shipping-like search-only semantics.
