...
- `runtime_current` ships the guarded `ProV2` path, not a raw always-on turn engine.
- Opening-book and early-white fallback guards are intentional parts of the shipping path.
- Promotion proof is direct Pro-vs-current evidence: current Pro, Normal, and Fast all must clear `win_rate >= 0.90`, `confidence >= 0.99`, and `candidate_avg_move_ms <= 700`.
...
- `primary_black_mana_bridge_ply20`: retained black injected mana-bridge seam on current baseline `l4,1;l5,0;mb`.
- `primary_black_spirit_bridge_ply19`: retained black injected spirit-bridge seam on the same current baseline.
...
- Kill any line that clears retained fixtures but does not move direct duel evidence against `runtime_current`.
