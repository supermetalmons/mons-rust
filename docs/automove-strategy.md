...
The next useful implementation should expose a policy-winner/context aggregate or a test-only selector over that complete policy set, then run the promotion dashboard. Only after that selector is strong on both panels should runtime code change. The runtime change should be a shared utility or root-evaluation feature that explains the repeated policy choice across panels, not another wrapper toggle, variant gate, local portfolio oracle, or active-context selector.
