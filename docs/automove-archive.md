...
- Durable outcome: keep `race_delta` as diagnostics, but archive before/after race deltas as no-source. The next root-pool iteration needs a genuinely new non-race feature family, such as root event or trajectory geometry, before another discriminator replay.
