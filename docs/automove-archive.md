...
- Durable outcome: keep the Outcome Corpus V2 contrast report as workbench infrastructure, but do not write runtime selectors from active-only contrast families. The next useful work is a focused sampled Pro validation of the active contrast family shortlist, ideally after adding a `--contrast-families` filter so the report can test those families without broad noisy output.
