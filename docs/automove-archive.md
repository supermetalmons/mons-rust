...
## Apr 9, 2026: Seed v36 Replay Killed Before Code Edits

- What was tried: refreshed `smart_automove_pro_reliability_duel_trace_probe` with `SMART_PRO_RELIABILITY_SEED_TAG=pro_turn_planner_reliability_v36` to test whether the retained white safe-progress seam from `v35` would carry into another bucket or whether the wall had already moved again.
- Why it stopped: direct Pro did go clean, but the losing buckets fragmented immediately. Direct Pro finished `0` regressions / `3` improvements / `9` flat, Normal `3` / `3` / `6`, and Fast `4` / `0` / `8`. Every exact move pair still stayed at count `1`: Normal only mixed one-off black mana-bridge `l0,5;l1,4` vs current `l4,1;l5,0;mb`, one-off black `ManaTempo` tie `l2,7;l2,8` vs `l2,7;l1,8`, and one-off white forced-prepass `l8,4;l8,5` vs `l8,4;l8,3`; Fast only mixed one-off early-black `negative_deny` `l0,5;l1,6` vs `l1,5;l3,6;l2,7` with one-off white `l8,5;l8,4` sibling reranks against `l8,5;l9,4` and `l8,5;l7,4`. The repeated `primary_white_safe_progress_rerank_ply27` story from `v35` did not carry forward at all.
- Durable lesson: do not spend from a seed just because direct Pro goes clean. If the losing buckets immediately collapse back into count-`1` black bridge / early-black / white forced-prepass / white sibling churn, keep only the note and wait for a repeated exact seam in the actual losing buckets.
