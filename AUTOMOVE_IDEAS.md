...
Current next sequence: do not write runtime code or a ProV4 comparator. Use the new `corpus_axis_summary` on an explicit active-blocker Pro slice, because the unfiltered eight-state cap was consumed by sampled `vs_shipping_pro` states and stayed baseline-save-risk instead of producing cross-panel evidence.
...
Log that command and read it with `scripts/summarize-automove-policy-matrix-log.py <log>` before raw records. Read `corpus_decision`, `next_action`, `source_blocker`, `corpus_axis_summary`, `route_buckets`, `record_filters`, and `coverage_gap_entries` first. If active stays `coverage_gap`, inspect `corpus_axis_summary.top_no_policy_axes` and `coverage_gap_entries` before any raw records and decide whether the next retained change is a lower-level feature report or a ProV4/root-feature probe. Runtime source stays untouched unless the summarizer reports `inspect_for_source`.
