...
Current next sequence: do not write runtime selector code and do not widen active cross-budget axes again without a new below-policy feature. Run one sampled cross-budget source-status pass to see whether the current corpus axes produce any sampled source candidates or only singleton/fragmented residue; read `cross_budget_axis_summary.source_status_counts`, `source_candidate_rollups`, and `blocked_candidate_rollups` before raw records.

```sh
rg "cross_budget_axis_summary|source_candidate_rollups|blocked_candidate_rollups" \
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-cross-budget-axis-sampled.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  /tmp/automove-cross-budget-axis-sampled.log
```

Runtime source stays untouched unless `source_candidate_rollups` is non-empty on sampled evidence and a future active rerun with a new below-policy feature also clears baseline-save, coverage-gap, singleton, and fragmentation statuses.
