...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, the forced-root pool JSONL discriminator on old fields, after-root race geometry, before/after race delta, or root trajectory/action geometry as source evidence. The retained root-pool provenance workbench is useful infrastructure, but current forced-root fields, race geometry, race deltas, and trajectory/action/zone/event-count geometry are no-source: the refreshed active Pro three-board slice still has `49` printed/tested roots, `12` wins, `37` losses, `oracle_decision=fragmented_root_features`, `clean_repeated_winner_provenance_count=0`, and the trajectory JSONL discriminator found zero clean repeated winner signals for exact axes, tokens, or token pairs. The next useful work is a postprocess-only root-pool contrast report, not another scalar selector feature: for each winning root, compare same-board losing siblings with nearest family/rank/path/trajectory/race/safety signatures and rank the fields or tokens that differ. Use that contrast report to choose the next feature family or return to Outcome Corpus V2; do not promote from contrast output alone.
...
# Add one postprocess-only contrast mode to
# scripts/summarize-automove-forced-root-pool-jsonl.py. It should read
# forced_root_pool_root/axis rows and emit per-label winner-vs-nearest-loser
# deltas. Then replay the same three active Pro coverage-gap boards directly;
# policy-matrix recovery is unnecessary unless the boards change.
while IFS='|' read -r ply label fen; do
  SMART_PRO_FORCED_ROOT_ORACLE_FEN="$fen" \
  SMART_PRO_FORCED_ROOT_ORACLE_LABEL="$label" \
  SMART_PRO_FORCED_ROOT_ORACLE_CONTINUATION=frontier_pro_v2_guarded \
  SMART_PRO_FORCED_ROOT_ORACLE_ROOT_SOURCE=frontier_pro_v2_guarded \
  SMART_PRO_FORCED_ROOT_ORACLE_START_PLY="$ply" \
  SMART_PRO_FORCED_ROOT_ORACLE_MAX_PLIES=56 \
  SMART_PRO_FORCED_ROOT_ORACLE_ROOT_LIMIT=24 \
  cargo test --release --lib smart_automove_pro_forced_root_oracle_probe -- --ignored --nocapture \
    > "/tmp/automove-root-pool-contrast-${label}.log" 2>&1
done <<'EOF'
7|active_outer_edge_candidate_white_ply7|0 0 w 0 0 1 0 0 3 n05d0xa0xn04/n08e0xn02/n06s0xn04/n04y0xn03xxmn02/xxmxxmn08xxm/xxQxxmn03xxUn03xxMxxQ/xxMxxMn07xxMxxM/n11/n07Y0xn03/n04A0xS0xn05/n03E0xn01D0xn05 8
20|active_outer_edge_candidate_white_ply20|0 1 w 0 0 0 0 0 5 n06a0xn03d0x/n08e0xn02/n06s0xn04/n04y0xn04xxmn01/xxmxxmn09/xxQxxmn03xxUn03xxMxxQ/xxMn08xxMxxM/n02xxME0xn07/n05S0xn01Y0xn03/n11/n04A0xD0xn05 8
40|active_outer_edge_candidate_white_ply40|0 1 w 1 0 4 1 0 7 n06a0xn03d0x/n11/n08xxmn02/n06s0xn04/xxmxxmn02y0xn06/E0xxxmn03xxUn04e0x/xxMn05S0xn01xxMxxMxxM/n02xxMD0xn07/n03A0xn03Y0xn03/n11/n11 8
EOF

scripts/summarize-automove-forced-root-oracle-log.py \
  --compact \
  --jsonl-out /tmp/automove-root-pool-contrast-three-board.jsonl \
  /tmp/automove-root-pool-contrast-active_outer_edge_candidate_white_ply*.log \
  > /tmp/automove-root-pool-contrast-three-board-summary.json

scripts/summarize-automove-forced-root-pool-jsonl.py \
  --compact \
  --contrast-report \
  --limit 40 \
  /tmp/automove-root-pool-contrast-three-board.jsonl
```
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench or the forced-root/root-pool diagnostic surface. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, the widened vulnerable-baseline token lead stayed fragmented, the utility-component exact/token/token-pair passes stayed fragmented or singleton, and the root-pool provenance plus forced-root pool JSONL discriminator passes produced only singleton or contaminated winner signals. After-root race geometry, before/after race deltas, and root trajectory/action geometry are now archived. If root-pool work continues, add the postprocess contrast report before inventing another feature.
