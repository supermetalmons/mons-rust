...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, or root-safety cross-budget passes. The next useful feature work is a root-pool / forced-root diagnostic view, not another selector: add richer per-root forced-oracle axes for detailed safety, reply/followup floor, advisor/path status, and root winner-vs-nonwinner separability, then validate on the existing corrected-horizon active Pro coverage-gap boards before considering any ProV4 comparator.

```sh
rg "FORCED_ROOT_ORACLE_ROOT|summarize-automove-forced-root-oracle-log|root_reply_risk_snapshot|pro_v2_spirit_followup_floor_score|safety_detail|ProPolicyMechanismRootClass" \
  src/models/automove_experiments/tests/diagnostics/mod.rs \
  scripts/summarize-automove-forced-root-oracle-log.py

SMART_PRO_FORCED_ROOT_ORACLE_LABEL=active_pro_outer_edge_ply7_guarded_roots \
...
cargo test --release --lib smart_automove_pro_forced_root_oracle_probe -- --ignored --nocapture \
  > /tmp/automove-forced-root-feature-axis-smoke.log 2>&1

scripts/summarize-automove-forced-root-oracle-log.py \
  /tmp/automove-forced-root-feature-axis-smoke.log
```
...
- Future source-bearing work should start by adding root-pool or forced-root feature evidence below current timing/continuation, root-preservation, reply-floor, root-safety, and policy/branch labels, then use that evidence to justify either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
