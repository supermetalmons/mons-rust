...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, or reply-floor cross-budget passes. Add the next Outcome Corpus V2 feature axis below policy/branch labels: detailed post-root safety deltas crossed with progress/setup class, so the corpus can tell whether broad `safe` / `vulnerable` buckets hide handoff, roundtrip, walk-vulnerability, or exact vulnerable-progress differences. Validate the new axis on one sampled and one active cross-budget state before considering any ProV4 comparator.

```sh
rg "own_drainer_vulnerable|own_drainer_walk_vulnerable|mana_handoff_to_opponent|has_roundtrip|safety_progress|mechanism_axes|baseline_better_mechanism_axes|PRO_POLICY_MATRIX_CORPUS_RECORD" \
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-root-safety-feature-axis-smoke.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  /tmp/automove-root-safety-feature-axis-smoke.log
```
...
- Future source-bearing work should start by adding a new Outcome Corpus V2 axis below current timing/continuation, root-preservation, reply-floor, and policy/branch labels, then use that evidence to justify either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
