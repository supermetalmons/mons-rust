...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, the forced-root pool JSONL discriminator on old fields, after-root race geometry, before/after race delta, root trajectory/action geometry, the same root-pool contrast replay, or the same active-only outcome contrast as source evidence. The retained Outcome Corpus V2 contrast report is useful infrastructure, but the active Pro four-state slice stayed `coverage_gap` and the contrast report returned `fragmented_repeated_outcome_contrast`: `3` candidate records, `3` same-outcome contrasts, `112` candidate-only signals, `13` repeated candidate-only signals, `0` low-fragmentation repeated signals, `54` candidate-only families, `28` repeated families, and `0` low-fragmentation repeated families. The next useful work is one focused sampled validation of those active contrast families before either archiving them or designing a below-fragmentation feature.
...
# Add one --contrast-families option to
# scripts/summarize-automove-outcome-jsonl.py so active-only contrast leads can
# be validated without rereading broad noisy families. Then run the sampled Pro
# slice against the active repeated-family shortlist; archive the lead unless it
# produces low-fragmentation repeated contrast evidence.
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-outcome-contrast-sampled-pro.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  --compact \
  --jsonl-out /tmp/automove-outcome-contrast-sampled-pro.jsonl \
  /tmp/automove-outcome-contrast-sampled-pro.log \
  > /tmp/automove-outcome-contrast-sampled-pro-summary.json

scripts/summarize-automove-outcome-jsonl.py \
  --compact \
  --contrast-report \
  --contrast-families advisor,primary,root_safety_detail,safety_progress,score_delta,winner_vs_baseline_primary,candidate_branch,decision_stage,family,followup_delta,rank,role,winner_reply_floor \
  --exclude-families root_pool,root_trajectory,race_delta,race_shape \
  /tmp/automove-outcome-contrast-sampled-pro.jsonl
```
