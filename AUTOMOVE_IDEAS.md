...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, or family-overlap passes. The next useful work is a JSONL state-level discriminator drilldown on the two shared contaminated candidate states from the family-overlap report: active `outer_edge_mana_rows` candidate-white and sampled `inner_wedge_mana_rows` candidate-white. Compare all candidate, baseline-save, and no-policy axes inside those states before adding a below-policy feature or archiving the families.
...
Use `scripts/summarize-automove-outcome-jsonl.py <export.jsonl>` after export to rank source candidates, blocked candidate-bearing axes, blocker counts, blocked axis-family rollups, and the default family-overlap drilldown directly from normalized rows.
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench. The clean-looking families already share the same contaminated candidate states; next inspect state-level differences inside those two states before justifying either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
