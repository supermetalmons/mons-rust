...
- This iteration kept no runtime code and added `black_progress_setup_shortlist_economics_probe` to test that economic separator directly:
  - On `black_progress_vs_setup_residue`, shipping's setup root is `436` score below the safe-progress incumbent and `10` root-rank slots back; its selected utility and reply floor are both `933` worse. The better-ranked setup sibling is only `2` rank slots back but is `839` search-score and `868` utility/reply-floor behind.
  - Widening the reply-risk margin through `436`, `839`, and `1024` admits the setup roots into the shortlist, but both the raw reply-risk guard and full ProV2 advisor still choose safe progress `l7,1;l9,3`.
  - On the retained `black_confirm_fast_setup` board, the kept setup root is score-tied with safe progress and already in the shortlist; the raw guard prefers safe progress, but the existing advisor family override correctly returns setup.
  - This kills the simple shortlist-widening/reentry follow-up. The only remaining black progress-vs-setup path would have to explain or change the underlying utility/reply-floor scoring that says the residue's safe-progress root is materially better.
- This iteration kept no runtime code and added `black_progress_reply_floor_breakdown_probe` to split that underlying utility gap:
  - On `black_progress_vs_setup_residue`, safe progress and setup share the same explicit scoring terms: confirmed score, score-path progress, immediate score window, and reply tactical flags are effectively identical. The gap comes from `residual_board_state`: safe progress is about `+843` better after the root and remains about `+843` to `+862` better after the worst replies.
...
- This iteration kept no runtime code and added `black_recovery_branch_reply_floor_attribution_probe` to split the remaining Pro no-go below the selector-ordering layer:
  - On `black_recovery_branch`, the live config (`enable_static_exact_evaluation=false`) gives the approved spirit root a small search-eval edge over shipping after root (`+11`) and after the quiet worst reply (`+101`), even though static-exact/raw breakdown scoring favors shipping by `-772` after root and `-349` after worst reply.
  - Toggling local scoring context, attack-reach summary, attack-target narrowing, and drainer-target narrowing is inert. The only switch that changes the ordering is static exact: `exact_on_*` variants favor shipping, while `exact_off_*` variants match the live approved-spirit ordering.
  - The same probe confirms shipping beats the score-leading mana sibling by `+19` with static exact on or off, so the earlier wrong-mana fallback remains avoidable; the unresolved problem is specifically spirit reentry versus shipping mana under non-static-exact search evaluation.
  - Do not convert this into a local black-recovery override yet. Prior direct black-recovery fallbacks already aligned this board and still failed retained reliability, so the useful new hypothesis is exact-static scoping/cost, not another board-local shipping mirror.
...
- For `black_recovery_branch`, the only fresh signal is static exact evaluation. Reopen it only with a scoped exact-static experiment that first proves runtime cost and retained reliability; do not reapply the old shortlist-local, guarded-legacy, or direct shipping fallback variants.
