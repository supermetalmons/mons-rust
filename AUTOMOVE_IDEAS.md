...
- Current diagnostic hypothesis: there is no source-level selector yet. The broad reset route scan found no clean low-fragmentation route; the filtered safety/progress and `engine_post_search` routes are retired as source evidence because their matching states split by policy, color, branch, first move, and advisor status. Outcome Corpus V2 now has a log summarizer with `corpus_decision` / `next_action` / `source_blocker`, multi-log `log_rollup` including rollup-level decisions, compact `coverage_gap_entries`, same-opening sibling summaries, record-level `corpus_axis_summary.top_axes_by_decision`, per-token `axis_filter_matches`, and a true total-state cap. A corrected-horizon forced-root oracle on the active Pro no-policy coverage-gap boards found winning roots in the current root set, but the forced-root digest found no repeated axis that separates those winners from losing roots. The widened active Pro same-opening pairing check stayed singleton, the sampled eight-state record-bearing slice stayed baseline-save-risk, the explicit active Pro axis summary stayed coverage-gap, and the focused sampled Pro axis-filter check killed the two active repeated-candidate leads. Work remains Outcome Corpus V2 feature extraction rather than runtime selectors.
...
Current next sequence: do not write runtime selector code, do not rerun the retired active axis filters as promotion work, and do not add a ProV4 comparator before feature records exist. Add one Outcome Corpus V2 field family for decision-stage timing and cheap continuation stability, then validate that the new fields print on a tiny record-bearing corpus slice.

```sh
rg "PRO_POLICY_MATRIX_CORPUS_RECORD|mechanism_axes|first_diff_ply|candidate_move|baseline_move|frontier_branch|candidate_branch" \
...
SMART_PRO_POLICY_MATRIX_TOTAL_STATE_LIMIT=1 \
...
After adding fields, log the tiny validation command and read it with `scripts/summarize-automove-policy-matrix-log.py <log>` before raw records. Confirm the digest exposes the new timing/continuation field family on `PRO_POLICY_MATRIX_CORPUS_RECORD` rows without changing `corpus_decision`, route permissions, or portfolio outcome counts. Only after that should a wider sampled/active Pro run ask whether the field separates candidate-only wins from baseline saves. Runtime source stays untouched unless the summarizer reports `inspect_for_source` from a low-fragmentation source candidate.
...
- Future source-bearing work should start by adding Outcome Corpus V2 timing/continuation feature fields, then use that evidence to justify either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
