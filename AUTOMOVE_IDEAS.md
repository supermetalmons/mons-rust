...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, the forced-root pool JSONL discriminator on old fields, after-root race geometry, or before/after race delta. The retained root-pool provenance workbench is useful infrastructure, but current forced-root fields, absolute race geometry, and race deltas are no-source: the refreshed active Pro three-board slice still has `49` printed/tested roots, `12` wins, `37` losses, `oracle_decision=fragmented_root_features`, `clean_repeated_winner_provenance_count=0`, and the race-delta JSONL discriminator found zero clean repeated winner signals for exact axes, tokens, or token pairs. The next useful work must introduce exactly one genuinely new root-pool feature outside current safety/reply/advisor/path/rank/family/utility/race geometry; prefer test-only root event or trajectory geometry, then replay the same three corrected boards and run the retained JSONL discriminator against the new rows.
...
# After adding exactly one new non-race root-pool feature to the cfg(test)-only
# oracle or JSONL export, recover the current compact
# coverage-gap divergence boards.
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-root-pool-coverage-gap-refresh.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  --compact \
  /tmp/automove-root-pool-coverage-gap-refresh.log \
  > /tmp/automove-root-pool-coverage-gap-refresh-summary.json

jq -r '.coverage_gap_entries[0].divergences[] | [.first_diff_ply,.board] | @tsv' \
  /tmp/automove-root-pool-coverage-gap-refresh-summary.json

# Run smart_automove_pro_forced_root_oracle_probe once per listed ply/FEN with
# guarded continuation/source, max plies 56, root limit 24, and labels
# active_outer_edge_candidate_white_ply<first_diff_ply>, then summarize:
scripts/summarize-automove-forced-root-oracle-log.py \
  --compact \
  --jsonl-out /tmp/automove-root-pool-provenance-three-board.jsonl \
  /tmp/automove-root-pool-provenance-ply*.log \
  > /tmp/automove-root-pool-provenance-three-board-summary.json

scripts/summarize-automove-forced-root-pool-jsonl.py \
  --compact \
  --limit 20 \
  /tmp/automove-root-pool-provenance-three-board.jsonl
```
