...
Current next sequence: do not write runtime selector code and do not add a ProV4 comparator from the current axes. Widen the new Outcome Corpus V2 cross-budget axis view to two active-blocker states per Pro/Normal/Fast duel, then read `cross_budget_axis_summary` before raw records. Use `SMART_PRO_POLICY_MATRIX_STATE_LIMIT=2` here because the goal is one aligned slice per budget; `SMART_PRO_POLICY_MATRIX_TOTAL_STATE_LIMIT` would cap globally and can hide budgets.

```sh
rg "cross_budget_axis_summary|cross_budget_seed_family|timing_continuation_axes" \
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-cross-budget-axis-widen.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  /tmp/automove-cross-budget-axis-widen.log
```

Read `cross_budget_axis_summary.cross_budget_decision_counts`, `top_axes_by_decision`, and `top_axis_rollups_by_decision`. Runtime source stays untouched unless the summarizer shows repeated `all_budget_repair` or clean `non_regressing_repair` axes with no baseline-save risk, no coverage gap, and low policy/branch/pair fragmentation.
...
- Future source-bearing work should start by widening the Outcome Corpus V2 cross-budget axis view over timing/continuation records, then use that evidence to justify either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
