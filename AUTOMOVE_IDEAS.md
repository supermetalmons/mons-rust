...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, or vulnerable-baseline token passes. The widened vulnerable-baseline lead is archived: `baseline_detail=vulnerable`, `baseline_safety=vulnerable`, and `baseline_signal=vulnerable_progress` stayed clean from baseline/no-policy rows but fragmented by policy, branch, first-move pair, and same-outcome evidence. The next useful work is a new diagnostics-only utility/root-evaluation feature source below current safety/progress labels: add corpus axes that bucket baseline-vs-winner `TurnEngineUtility` component deltas and rank/score margin shapes, then run one small sampled+active smoke. Runtime source stays untouched unless that new axis creates non-empty row-level source candidates and later active/cross-budget gates clear baseline-save, coverage-gap, singleton, and fragmentation blockers.
...
# After adding diagnostics-only utility/root-evaluation axes to policy-matrix corpus records:
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-utility-component-axis-smoke.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  --compact \
  --jsonl-out /tmp/automove-utility-component-axis-smoke.jsonl \
  /tmp/automove-utility-component-axis-smoke.log \
  > /tmp/automove-utility-component-axis-smoke-summary.json
...
scripts/summarize-automove-outcome-jsonl.py \
  ...
  /tmp/automove-utility-component-axis-smoke.jsonl
```
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, and the widened vulnerable-baseline token lead stayed fragmented. The next feature source should be utility/root-evaluation component deltas below current safety/progress labels, not another selector or token pass over existing axes.
