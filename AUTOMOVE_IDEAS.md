...
Current next sequence: do not write runtime code or a ProV4 comparator. Test the two repeated active-only candidate axes against sampled Pro with a focused record-axis filter before designing any feature from them. The active Pro slice stayed `coverage_gap`, but `corpus_axis_summary.top_axes_by_decision.repeated_candidate_axis` exposed two candidate-only leads with no active baseline-better states: safe-step to spirit-development progress, and selected-root to preaccept/legacy/full-pool role.
...
SMART_PRO_POLICY_MATRIX_RECORD_AXIS_FILTER='axis=safety_progress baseline_safety=safe baseline_progress=safe_step_progress winner_safety=safe winner_progress=spirit_development,axis=role baseline_role=selected baseline_live=top3_live winner_role=pre_accept+legacy+legacy_full_pool winner_live=top3_live' \
...
Log that command and read it with `scripts/summarize-automove-policy-matrix-log.py <log>` before raw records. Read `corpus_decision`, `next_action`, `source_blocker`, `corpus_axis_summary.top_axes_by_decision`, `record_filters`, and `route_buckets` first. If either active lead becomes `baseline_save_risk` or `baseline_better_only` on sampled Pro, archive it and return to a ProV4/root-feature or continuation-stability feature path. Runtime source stays untouched unless the summarizer reports `inspect_for_source`.
