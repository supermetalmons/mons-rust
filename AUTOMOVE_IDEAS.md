...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, or utility/rank token-pair passes. The focused `utility_component_delta,rank_score_delta` token-pair discriminator is archived as no-source: it found `184` candidate token pairs, `9` clean repeated candidate pairs, `0` low-fragmentation repeated candidate pairs, `52` clean singleton pairs, and `123` contaminated pairs; the repeated clean pairs were all fragmented by policy, branch, first-move pair, and/or same-outcome evidence. The next useful work is diagnostics-only ProV4/root-pool provenance: emit a queryable per-root pool view for the current coverage-gap boards so the next feature can explain candidate roots below policy labels before any comparator or runtime selector is written.
...
# After adding diagnostics-only root-pool provenance rows/summaries for all considered roots:

SMART_PRO_FORCED_ROOT_ORACLE_LABEL=active_outer_edge_candidate_white_ply7 \
SMART_PRO_FORCED_ROOT_ORACLE_MODE=pro \
...
SMART_PRO_FORCED_ROOT_ORACLE_OPPONENT_MODE=pro \
...
cargo test --release --lib smart_automove_pro_forced_root_oracle_probe -- --ignored --nocapture \
  > /tmp/automove-root-pool-provenance-smoke.log 2>&1

scripts/summarize-automove-forced-root-oracle-log.py \
  --compact \
  /tmp/automove-root-pool-provenance-smoke.log \
  > /tmp/automove-root-pool-provenance-smoke-summary.json
```
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench or the forced-root/root-pool diagnostic surface. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, the widened vulnerable-baseline token lead stayed fragmented, and the utility-component exact/token/token-pair passes stayed fragmented or singleton. Move away from current policy-corpus axes toward a deeper root-pool/ProV4 diagnostic before any further runtime selector work.
