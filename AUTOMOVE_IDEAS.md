...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, the forced-root pool JSONL discriminator on old fields, or the after-root race-geometry feature by itself. The retained root-pool provenance workbench is useful infrastructure, but current forced-root fields plus after-root race geometry are no-source: the refreshed active Pro three-board slice still has `49` printed/tested roots, `12` wins, `37` losses, `oracle_decision=fragmented_root_features`, `clean_repeated_winner_provenance_count=0`, and the new race JSONL discriminator found zero clean repeated winner signals for exact axes, tokens, or token pairs. The next useful work must first add a true before/after forced-root race-delta feature under the test harness or summarizer export, then replay the same three corrected boards and run the retained JSONL discriminator against the new rows.
...
# After adding exactly one before/after forced-root race-delta feature to the
# cfg(test)-only oracle or JSONL export, recover the current compact
