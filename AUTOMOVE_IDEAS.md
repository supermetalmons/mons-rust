...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, or state-discriminator passes. The next useful work is a JSONL axis-token discriminator inside the two contaminated target states: active `outer_edge_mana_rows` candidate-white and sampled `inner_wedge_mana_rows` candidate-white. Tokenize axis key/value fields within the contaminated families and look for candidate-only tokens that repeat across both states while staying absent from baseline-save and no-policy rows. If no repeated token-level discriminator appears, archive these family axes and switch to a new below-policy feature source.

After adding that token drilldown to the JSONL script, validate it with:
...
Use `scripts/summarize-automove-outcome-jsonl.py <export.jsonl>` after export to rank source candidates, blocked candidate-bearing axes, blocker counts, blocked axis-family rollups, default family-overlap drilldown, and state-discriminator rows directly from normalized rows.
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench. The clean-looking families already share the same contaminated candidate states, and state-level exact-axis differences did not produce a unique candidate family. Next inspect token-level differences inside those contaminated families before justifying either a corpus-calibrated utility feature or a test-only ProV4 unified root policy.
