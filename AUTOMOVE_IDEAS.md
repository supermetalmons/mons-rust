...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, or the forced-root pool JSONL discriminator on the same current fields. The retained root-pool provenance workbench is useful infrastructure, but current forced-root fields are no-source: the refreshed active Pro three-board slice reproduced `49` printed/tested roots, `12` wins, `37` losses, `oracle_decision=fragmented_root_features`, `clean_repeated_winner_provenance_count=0`, and `1963` JSONL rows; the new JSONL discriminator found zero clean repeated winner signals for exact axes, tokens, or token pairs. The next useful work must first add a genuinely new forced-root/root-pool feature under the test harness or summarizer export, then replay the same three corrected boards and run the retained JSONL discriminator against the new rows.
...
# After adding exactly one new forced-root/root-pool feature to the
