...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, or the single ply-7 root-pool provenance smoke. The retained root-pool provenance workbench is useful infrastructure, but the ply-7 active Pro coverage-gap run is not source permission: it found `7/16` winning roots, `9` losses, `oracle_decision=fragmented_root_features`, `clean_repeated_winner_provenance_count=0`, and `641` JSONL rows. The next useful work is a multi-board root-pool provenance refresh over the three corrected active Pro coverage-gap divergence boards, using `coverage_gap_entries` to recover any FENs not preserved in docs, then summarizing all forced-root logs together.
...
# First recover the current compact coverage-gap divergence boards.
...
jq -r '.coverage_gap_entries[0].first_divergences[] | [.first_diff_ply,.board] | @tsv' \
  /tmp/automove-root-pool-coverage-gap-refresh-summary.json

# Then run smart_automove_pro_forced_root_oracle_probe once per listed ply/FEN
# with guarded continuation/source, max plies 56, root limit 24, and labels
# active_outer_edge_candidate_white_ply<first_diff_ply>. Summarize all logs:
scripts/summarize-automove-forced-root-oracle-log.py \
  --compact \
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench or the forced-root/root-pool diagnostic surface. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, the widened vulnerable-baseline token lead stayed fragmented, the utility-component exact/token/token-pair passes stayed fragmented or singleton, and the first ply-7 root-pool provenance pass produced only singleton or contaminated winner provenance. Move away from current policy-corpus axes toward a multi-board root-pool/ProV4 diagnostic before any further runtime selector work.
