...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, exact utility-component axis, utility/rank token, utility/rank token-pair, the single ply-7 root-pool provenance smoke, the three-board root-pool provenance refresh, the forced-root pool JSONL discriminator on old fields, after-root race geometry, before/after race delta, root trajectory/action geometry, or the same root-pool contrast replay as source evidence. The retained root-pool contrast report is useful infrastructure, but the checked active Pro three-board slice had `49` roots, `12` wins, `37` losses, zero repeated exact field deltas, zero repeated winner-field values, and only token-only repeated contrasts from archived trajectory atoms. The next useful work is a postprocess-only Outcome Corpus V2 contrast report: compare candidate-only policy-axis records against nearest baseline-better, no-policy, and same-outcome sibling records by state/opening/budget, excluding archived root-pool/race/trajectory fields, then decide whether a non-root-pool feature family deserves a new diagnostic.
...
# scripts/summarize-automove-outcome-jsonl.py. It should read policy_axis and
# policy_decision rows, compare candidate-only records against nearest
# baseline-better/no-policy/same-outcome siblings, and emit only diagnostic
# contrast counts.
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-outcome-contrast.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  --compact \
  --jsonl-out /tmp/automove-outcome-contrast.jsonl \
  /tmp/automove-outcome-contrast.log \
  > /tmp/automove-outcome-contrast-summary.json

scripts/summarize-automove-outcome-jsonl.py \
  --compact \
  --contrast-report \
  --exclude-families root_pool,root_trajectory,race_delta,race_shape \
  /tmp/automove-outcome-contrast.jsonl
```
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, the widened vulnerable-baseline token lead stayed fragmented, the utility-component exact/token/token-pair passes stayed fragmented or singleton, and the root-pool provenance, forced-root pool JSONL discriminator, race/trajectory passes, and root-pool contrast report produced only singleton, contaminated, or archived-token-only winner signals. Do not invent another root-pool scalar until an outcome-level contrast points to a non-archived feature family.
