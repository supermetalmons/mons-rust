...
- The two remaining local seams now both sit in known no-go classes:
  ...
  - Treat the live white residue as a frontier-`ProV2` rerank own-search-breadth split, not just a selector-disabled ordering seam or a missing recovery root from the frontier root set.
