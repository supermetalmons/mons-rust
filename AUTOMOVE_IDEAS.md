...
Current next sequence: do not write runtime selector code and do not rerun current-axis, root-preservation, reply-floor, root-safety, forced-root feature-axis, family-overlap, state-discriminator, broad all-family token, vulnerable-baseline token, or exact utility-component axis passes. The first `axis=utility_component_delta` smoke is archived as no-source: exact utility axes were singleton or fragmented, family overlap was shared but contaminated, and the only clean repeated focused utility token was `eval_score=winner_better_1_95`, still fragmented by policy, branch, first-move pair, and same-outcome evidence. The next useful work is postprocess-only: add a JSONL token-pair / token-conjunction discriminator for focused axis families, then use it on `utility_component_delta,rank_score_delta` before adding any new runtime or harness axis. Runtime source stays untouched unless a focused token pair is repeated, clean from baseline/no-policy rows, and low-fragmentation.
...
# After adding a postprocess-only token-pair discriminator to the JSONL workbench:
...
cargo test --release --lib smart_automove_pro_policy_matrix_probe -- --ignored --nocapture \
  > /tmp/automove-utility-token-pair-smoke.log 2>&1

scripts/summarize-automove-policy-matrix-log.py \
  --compact \
  --jsonl-out /tmp/automove-utility-token-pair-smoke.jsonl \
  /tmp/automove-utility-token-pair-smoke.log \
  > /tmp/automove-utility-token-pair-smoke-summary.json

scripts/summarize-automove-outcome-jsonl.py \
  --compact \
  --limit 20 \
  --state-axis-limit 20 \
  --families utility_component_delta,rank_score_delta \
  --token-families utility_component_delta,rank_score_delta \
  /tmp/automove-utility-token-pair-smoke.jsonl
```
...
- Future source-bearing work should start from the Outcome Corpus V2 JSONL workbench. The clean-looking families already share the same contaminated candidate states, state-level exact-axis differences did not produce a unique candidate family, the widened vulnerable-baseline token lead stayed fragmented, and the first utility-component exact/token pass also stayed fragmented or singleton. The next source check should be a focused token-pair discriminator in postprocess; if that also fragments, move away from current policy-corpus axes toward a deeper root-pool/ProV4 diagnostic.
