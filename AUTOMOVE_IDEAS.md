...
Current next sequence: consume the record-axis filter detail rows on the current top clean-fragmented route before any broader postprocessor work. This is Outcome Corpus V2 validation only, not runtime source permission.
...
SMART_PRO_POLICY_MATRIX_RECORD_FILTER_DETAIL_LIMIT=16 \
...
- Latest reset evidence: the focused route-filter scans have oracle coverage but no source permission. The safety/progress and `engine_post_search` routes remain fragmented across policy, branch, color, budget, and first-move pair. The retained source change is harness-only record-filter detail output.
