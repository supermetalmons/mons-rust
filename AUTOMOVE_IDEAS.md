...
- Failed iteration:
  - `frontier_pro_v3_family_competition_guarded` paired a tighter black turn-6 spirit-reentry filter with a tighter white turn-3 mana sibling competition and a candidate-only turn-3 white recovery override. It did fix `vs_shipping_pro_black_recovery_branch` and `vs_shipping_pro_white_split_trace`, and it again moved `vs_shipping_normal_white_head_acceptance` at the advisor layer from vulnerable `l9,4;l8,3` to safe `DrainerSafetyRecovery l9,4;l8,5`, but `vs_shipping_pro_opening_reply_white` still stayed on `l10,10;l10,9` and the final selected root on `vs_shipping_normal_white_head_acceptance` still snapped back to vulnerable `l9,4;l8,3`, so the code was discarded before `pro-triage` or `runtime-preflight`.
...
- Tightening the black turn-6 spirit-reentry filter and the white turn-3 mana sibling competition together can align `vs_shipping_pro_black_recovery_branch` plus `vs_shipping_pro_white_split_trace`, but that shared package still leaves `vs_shipping_pro_opening_reply_white` unchanged and still lets `vs_shipping_normal_white_head_acceptance` snap back to vulnerable `l9,4;l8,3` after advisor approval moves to safe recovery.
- There is no second live challenger today.
...
- Do not reopen the combined black turn-6 reentry filter plus white split-trace competition package as a standalone challenger. Even when it fixes `vs_shipping_pro_black_recovery_branch` and `vs_shipping_pro_white_split_trace`, `vs_shipping_pro_opening_reply_white` can still stay unchanged and `vs_shipping_normal_white_head_acceptance` can still fail at final head acceptance.
