...
The same setting also emits `PRO_POLICY_MATRIX_GLOBAL_MECHANISM_ROUTE` for each separated axis. Read its `label` before opening raw records: `baseline_save_risk` kills selector work for that axis, `single_scope_repeat` is routing evidence only, and `cross_panel_clean` / `cross_budget_clean` earns a narrow corpus-record plus decision-probe rerun before source work. The route line includes the exact panels and duels that contributed candidate-only wins and baseline-better saves, so global-only reset scans no longer require manually reconstructing panel/budget coverage from raw logs.
